...
SELECT last_day_of_month."year_month2",
       ...
       CASE
           WHEN last_day_of_month."month" IN (3, 6, 9, 12)
               THEN 1
           ELSE 0 END                                                              AS is_last_in_quarter,
       CASE
           WHEN last_day_of_month."month" = 12
               THEN 1
           ELSE 0 END                                                              AS "is_last_in_year",
       ...
       last_day_of_month."month_name3" || ' ' ||
       last_day_of_month."year2"                                                   AS "fullname",
...
FROM calendar_dates last_day_of_month
    JOIN calendar_dates first_day_of_month
    ...
    JOIN (SELECT "year_month2",
          ...
          FROM calendar_dates
          GROUP BY "year_month2") dates_of_month
    ...
    JOIN (SELECT hours."year_month2",
          ...
          FROM calendar_hours hours
              JOIN (SELECT "year_month2",
                           ...
                           SUM(CASE WHEN "is_working_day" = 1 THEN 8 ELSE 0 END)       AS "working_hours"
                    FROM calendar_dates
                    GROUP BY "year_month2") dates
                  ON dates."year_month2" = hours."year_month2"
          GROUP BY hours."year_month2") first_and_last_hours
    ...
    JOIN (SELECT hours."year_month2_cet",
          ...
          GROUP BY hours."year_month2_cet") first_and_last_hours_cet
...
2020-01,2020 1,2020,1,1,January  ,01,JAN,2020-01-01,2020-01-31,31,744,2019-12-31,2019-12,0,0,23,8,1,22,176,2020-01-01 00,2020-01-31 23,2019-12-31 23,2020-01-31 22,2020-02-01,JAN 20,2020-02,1,9998
 */
/*
 sza(c)
 */
